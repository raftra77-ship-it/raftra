...
export const GrowthAnalysisSection: React.FC<GrowthAnalysisSectionProps> = ({
  ...
  // Derive connection state based on demo toggle
  const isMetaConnected = isDemoActive && (connectedSources.meta ?? true);
  const isGoogleConnected = isDemoActive && (connectedSources.google ?? true);
  const isGa4Connected = isDemoActive && (connectedSources.ga4 ?? true);
  const isGscConnected = isDemoActive && (connectedSources.gsc ?? true);
  const isShopifyConnected = isDemoActive && (connectedSources.shopify ?? true);
