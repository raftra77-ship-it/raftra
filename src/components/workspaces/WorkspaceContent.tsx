...
const STATUS_LABEL: Record<string, string> = {
  ...
  approved: 'Approved ΓÇö ready to send',
  rejected: 'Rejected',
...
export const WorkspaceContent: React.FC<{ workspaceId: number | null }> = ({ workspaceId }) => {
  ...
  const generate = async () => {
    ...
    if (!looksLikeRealTopic(topic)) {
      setTopicError("That doesn't look like a real topic. Enter a few words describing what to write about ΓÇö e.g. \"best data structures for coding interviews\".");
      return;
    }
    setTopicError(null);
    setLoading(true);
    try {
      ...
      setTopic('');
      // Generation runs in the background ΓÇö poll for the new draft.
      let tries = 0;
  ...
  // send fails we say so and leave the draft unchanged ΓÇö never mark it published falsely.
  const publish = async (id: number, target: string) => {
    ...
    setNotice(`${d.message || 'Sent successfully.'}${link ? ` ΓåÆ ${link}` : ''}`);
    loadDrafts();
  };
    ...
    <>
      <div className="glow-card" style={{ padding: '24px', marginTop: '24px' }}>
        <h3 style={{ fontSize: '14px', fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)', marginBottom: '6px' }}>
          CONTENT STUDIO ΓÇö GENERATE & REVIEW
        </h3>
        <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '16px', lineHeight: 1.6 }}>
          Enter a topic ΓåÆ AI writes a draft ΓåÆ you review, edit if needed, and approve ΓåÆ send it to your site.
          <strong style={{ color: '#fff' }}> Nothing publishes automatically</strong> ΓÇö it arrives as a pull
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: topicError ? '8px' : '20px' }}>
          ...
          >
            {loading ? 'GeneratingΓÇª' : 'Generate Content'}
          </button>
        </div>
        {topicError && (
        ...
        {drafts.length === 0 && (
          <div style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>
            No content yet. Enter a topic and generate an SEO article ΓÇö it will appear here for review.
          </div>
        )}
