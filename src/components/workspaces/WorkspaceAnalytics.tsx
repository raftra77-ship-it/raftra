        ...
        <div style={{ height: '180px', width: '100%', marginTop: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', padding: '12px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={roasBarData}>
              ...
              <Bar dataKey="ROAS" fill={SERIES[0]} radius={[4, 4, 0, 0]} maxBarSize={38} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      );
        ...
        <div style={{ height: '180px', width: '100%', marginTop: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', padding: '12px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={cpaData}>
              ...
              <Line type="monotone" dataKey="CPA" stroke={SERIES[1]} strokeWidth={2} dot={{ r: 4, strokeWidth: 0, fill: SERIES[1] }} activeDot={{ r: 5 }} isAnimationActive={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px', paddingBottom: '40px' }}>
      ...
      {/* 3. SEO & GEO PERFORMANCE.
          ...
          section containers (defined in App.css) — so the product's two SEO surfaces read as
          one family rather than one being a contained card and the other a bare heading over
          loose tiles. --pipe-accent is --success because that is already this section's
          accent: the Globe, the GEO series and the trend chips. */}
      <div className="glow-card seo-shell" style={{ padding: '24px', ['--pipe-accent' as string]: 'var(--success)' }}>
        ...
        {/* minmax(0,…) rather than 1fr 2fr: a bare 1fr floors at the content's min width and
            the recharts container refuses to shrink below it, pushing the grid wider than
            the shell. */}
                ...
                <div style={{ width: '100%', height: '240px' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={seoGeoData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Line type="monotone" dataKey="Organic" stroke={SERIES[0]} strokeWidth={2} dot={false} isAnimationActive={false} />
                      <Line type="monotone" dataKey="Impressions" stroke={SERIES[2]} strokeWidth={2} dot={false} isAnimationActive={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
