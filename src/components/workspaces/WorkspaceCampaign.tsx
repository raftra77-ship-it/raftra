...
const money = (v: any) => `Γé╣${Number(v || 0).toLocaleString('en-IN')}`;
const ago = (ts: number) => {
  const m = Math.floor((Date.now() - ts) / 60000);
  return m < 1 ? 'just now' : m === 1 ? '1 min ago' : `${m} mins ago`;
};

// ΓöÇΓöÇ shared styles ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// Short values sit on one line (label left, value right). Long values ΓÇö audience, placements ΓÇö
...
export const WorkspaceCampaign: React.FC<WorkspaceCampaignProps> = ({ workspaceId, creativeAssets = [] }) => {
  // ΓöÇΓöÇ the full ideation brief (all original fields restored) ΓöÇΓöÇ
  ...
  const whyRows: { label: string; value: string; reason: string }[] = [
    ...
    rec.budget_allocation && { label: 'Budget Split', value: `${rec.budget_allocation.meta_pct ?? 'ΓÇö'}% Meta / ${rec.budget_allocation.google_pct ?? 'ΓÇö'}% Google`, reason: rec.budget_allocation.reason },
  ...
  // ΓöÇΓöÇ generate strategy (+ its ad image, together) ΓöÇΓöÇ
  const generate = async () => {
    ...
    const prompt = `Create an ad campaign. Theme/focus: ${form.campaignFocus}. Objective: ${form.objective}. Total budget: Γé╣${form.budget}. Audience: ${form.audience}. Funnel: ${form.funnel}. Geo: ${form.placement} (${form.geoTargetingLevel}). Schedule: ${form.schedule}. Tracking: ${form.tracking}.`;
    try {
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        } else if (tries >= 20) { clearInterval(poll); setIsGenerating(false); flash('Taking longer than expected ΓÇö check the agent logs.', false); }
      } catch { /* keep polling */ }
    }, 3000);
  };
  ...
  const adSetup = async (platform: 'meta' | 'google', action: 'connect' | 'launch') => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        flash(`${P} ${action} done in MOCK mode ΓÇö no real ad account was touched.`);
  ...
  const publish = async (targets: ('meta' | 'google')[]) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        flash(d.message || `Published to ${label} (demo). It's now in Recently Published below ΓÇö starting a fresh campaign.`);
  ...
  // Reset the whole workflow so the top checklist (Ideation ΓåÆ Approve ΓåÆ ΓÇª) appears fresh.
  const resetFlow = () => {
  ...
  const connectMeta = async () => {
    try {
      ...
      flash(d.detail || 'Meta login isnΓÇÖt configured on the server yet (mock mode). You can still publish in demo mode.', false);
  ...
  const regenerateImage = async () => {
    ...
    try {
      ...
      flash('Generating a new image ΓÇö it appears in Creative Studio and your library.');
    } catch { flash('Could not start image generation.', false); }
    setBusy(null);
  };

  // ΓöÇΓöÇ presets (restored) ΓöÇΓöÇ
  const presets = [
  ...
  // ΓöÇΓöÇ import / export (restored) ΓöÇΓöÇ
  const exportCampaign = () => {
    if (!campaign) { flash('Nothing to export yet ΓÇö generate a strategy first.', false); return; }
    try {
      ...
      flash('Export failed ΓÇö could not build the file.', false);
    }
  };
  const importCampaign = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    r.onload = ev => {
      try {
        ...
        flash(d.strategy
          ? 'Campaign brief imported into the form on the left. Note: only the brief restores automatically ΓÇö click "Generate Strategy + Ad" to recreate its AI strategy and image.'
          : 'Campaign brief imported into the form on the left.');
      } catch { flash('Invalid campaign file ΓÇö could not read it as JSON.', false); }
    };
  ...
  // ΓöÇΓöÇ sample performance feed (mock data, kept at top) ΓöÇΓöÇ
  const sampleFeed = [
    { id: 'f1', tag: 'Scale (high performance)', icon: CheckCircle2, color: '#00e676', bg: 'rgba(0,230,118,0.08)', bd: 'rgba(0,230,118,0.3)', name: 'Diwali Festive Sale (Meta)', metrics: 'ROAS 4.2├ù  |  CPA Γé╣900  |  Spend Γé╣35,000', rec: 'Running 38% above ROAS target. Scale daily budget +25%.', act: 'Scale Budget +25%' },
    { id: 'f2', tag: 'Rotate creative (fatigue)', icon: AlertTriangle, color: '#ffb74d', bg: 'rgba(255,183,77,0.08)', bd: 'rgba(255,183,77,0.3)', name: 'Summer Apparel Retargeting', metrics: 'Frequency 4.1├ù  |  CTR 0.8%  |  Skip 74%', rec: 'Frequency hit 4.1├ù with a high skip rate. Rotate the creative.', act: 'Rotate Creative' },
    { id: 'f3', tag: 'Kill (CPA limit exceeded)', icon: OctagonX, color: '#ff5252', bg: 'rgba(255,82,82,0.08)', bd: 'rgba(255,82,82,0.3)', name: 'Broad TOF Awareness (Google)', metrics: `CPA Γé╣3,900 (cap Γé╣${smart.cpaThreshold})  |  0 conv (24h)`, rec: 'CPA breached your safety threshold. Auto-kill rule triggered.', act: 'Kill Ad Set' },
    { id: 'f4', tag: 'Pivot (audience saturation)', icon: RefreshCw, color: '#9c7bff', bg: 'rgba(156,123,255,0.08)', bd: 'rgba(156,123,255,0.3)', name: 'State-Level Scale Campaign', metrics: 'MH ROI 1.5├ù  |  KA ROI 3.8├ù', rec: 'Regional saturation detected. Reallocate 40% budget to high-ROI zones.', act: 'Reallocate Budget' },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={sectionHint}>Describe your campaign once ΓÇö AI writes the strategy and the ad image. You approve it, and that approved plan drives Meta, Google and publishing.</p>
        </div>
      ...
      {/* ΓöÇΓöÇ 1. performance & action feed (sample data) ΓöÇΓöÇ */}
      <div style={card}>
          ...
          {sampleFeed.map(f => {
            ...
            return (
              <div key={f.id} style={{ padding: '15px', background: f.bg, border: `1px solid ${f.bd}`, borderRadius: '11px', display: 'flex', flexDirection: 'column', gap: '10px', opacity: done ? 0.55 : 1 }}>
                ...
                <button onClick={() => { setExecuted(e => [...e, f.id]); flash('Example action ΓÇö connect a live ad account to apply this for real.', false); }}
                  disabled={done}
      ...
      {/* ΓöÇΓöÇ 2. the progress bar ΓöÇΓöÇ */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
          {steps.map((st, i) => (
            <React.Fragment key={st.n}>
              ...
              {i < steps.length - 1 && <span style={{ color: 'var(--text-muted)', flexShrink: 0 }}>ΓÇ║</span>}
            </React.Fragment>
          ))}
        </div>
      </div>

      {/* ΓöÇΓöÇ 3. brief + generated strategy (with its image) ΓöÇΓöÇ */}
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '18px' }}>
            {presets.map(p => {
              const Icon = p.icon;
              return (
                <button key={p.id} onClick={() => { setForm(f => ({ ...f, ...p.p })); flash(`"${p.title}" loaded ΓÇö tweak anything, then generate.`); }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div><span style={label}>Total budget (Γé╣)</span><input type="number" style={input} value={form.budget} onChange={e => setForm(f => ({ ...f, budget: Number(e.target.value) }))} /></div>
            </div>
            ...
            <button onClick={generate} disabled={isGenerating} style={{ ...btnPrimary, opacity: isGenerating ? 0.7 : 1, cursor: isGenerating ? 'wait' : 'pointer' }}>
              <Sparkles size={15} /> {isGenerating ? 'Writing your strategy & adΓÇª' : campaign ? 'Regenerate Strategy' : 'Generate Strategy + Ad'}
            </button>
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <p style={{ ...sectionHint, marginBottom: '16px' }}>
            {!campaign ? 'Nothing generated yet.' : approved
              ? 'Approved ΓÇö this exact plan is what Meta, Google and publishing below are using.'
          ...
          {!campaign ? (
            ...
            <>
              {/* the ad image ΓÇö pick the one creative that flows into the platform reviews */}
                  ...
                  <div style={{ flex: 1, minWidth: '160px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '9px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <button onClick={regenerateImage} disabled={busy === 'image'} style={{ ...btnGhost, padding: '8px 12px', fontSize: '12px' }}>
                        <RefreshCw size={13} /> {busy === 'image' ? 'GeneratingΓÇª' : 'New image'}
                      </button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '18px' }}>
                <div>
                  <Row k="Objective" v={campaign.objective || spec.objective || 'ΓÇö'} />
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    ...
                    {keywords.length === 0 && <span style={{ fontSize: '11.5px', color: 'var(--text-muted)' }}>ΓÇö</span>}
                  </div>
              ...
              {/* Audience is the longest field ΓÇö give it the full width so it reads as a sentence. */}
              ...
              {!approved ? (
                <button onClick={approve} disabled={busy === 'approve'} style={{ width: '100%', marginTop: '12px', background: '#00e676', border: 'none', color: '#03121a', padding: '13px', borderRadius: '9px', fontSize: '14px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  <ShieldCheck size={16} /> {busy === 'approve' ? 'ApprovingΓÇª' : 'Approve & send to all steps'}
                </button>
              ) : (
                <div style={{ marginTop: '12px', padding: '11px 14px', background: 'rgba(0,230,118,0.08)', border: '1px solid rgba(0,230,118,0.3)', borderRadius: '9px', fontSize: '12.5px', color: '#00e676', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <CheckCircle2 size={15} /> Approved ΓÇö passed to Meta, Google and Review below.
                </div>
              )}
            </>
          )}
      ...
      {/* ΓöÇΓöÇ 4. select platforms ΓöÇΓöÇ */}
      <Gated locked={!approved} why="Approve the strategy first to choose platforms">
        <div style={{ ...card, display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '215px', fontSize: '11.5px', color: 'var(--text-secondary)', background: 'rgba(90,82,255,0.06)', border: '1px solid rgba(90,82,255,0.2)', borderRadius: '10px', padding: '12px 14px', lineHeight: 1.55 }}>
            <b style={{ color: '#fff' }}>Edit each platform on its own.</b> Same approved strategy ΓÇö different ads per platform.
          </div>
        </div>
      </Gated>

      {/* ΓöÇΓöÇ 5. per-platform review ΓöÇΓöÇ */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          {platforms.meta && (
            <div style={card}>
              ...
              <Row k="Campaign name" v={campaign?.name || 'ΓÇö'} />
              <Row k="Objective" v={campaign?.objective || 'ΓÇö'} />
              <Row k="Budget" v={split.meta ? money(split.meta.amount) : 'ΓÇö'} />
              ...
              <div style={{ marginTop: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {selectedImage ? (
                    ...
                    <span style={{ fontSize: '11.5px', color: 'var(--text-muted)' }}>No creative selected ΓÇö approve one above.</span>
                  )}
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px', marginTop: '15px', flexWrap: 'wrap' }}>
                {!metaSetup.connected ? (
                  ...
                  <button onClick={() => adSetup('meta', 'launch')} disabled={busy === 'meta-launch'} style={{ ...btnPrimary, flex: 1, padding: '10px' }}>
                    <Check size={14} /> {busy === 'meta-launch' ? 'MarkingΓÇª' : 'Mark as Ready'}
                  </button>
                ) : (
          ...
          {platforms.google && (
            <div style={card}>
                  ...
                  <div style={{ display: 'flex', gap: '7px', marginTop: '8px', fontSize: '11.5px', color: 'var(--text-secondary)', lineHeight: 1.45 }}>
                    ...
                    <span>
                      {googleType === aiGoogleType
                        ...
                        : <>AI recommended <b style={{ color: '#b3aaff' }}>{aiGoogleType}</b> ΓÇö {rec.google_campaign_type.reason} You've switched to <b style={{ color: '#b3aaff' }}>{googleType}</b>.</>}
                    </span>
                  </div>
                )}
              </div>
              <Row k="Campaign name" v={campaign?.name || 'ΓÇö'} />
              <Row k="Budget" v={split.google ? money(split.google.amount) : 'ΓÇö'} />
              {gt.includes('search') && <Row k="Landing page" v={spec.landing_page || form.tracking || 'ΓÇö'} />}
              ...
              <Row k={`Headlines (${headlines.length})`} v={
                ...
                ) : <span style={{ color: 'var(--text-muted)' }}>ΓÇö</span>} stack />
              ...
              {gShow.images && <Row k="CTA" v={g.cta || (rec.cta && rec.cta.value) || 'ΓÇö'} />}
              <div style={{ display: 'flex', gap: '8px', marginTop: '15px', flexWrap: 'wrap' }}>
                {!googleSetup.connected ? (
                  ...
                  <button onClick={() => adSetup('google', 'launch')} disabled={busy === 'google-launch'} style={{ ...btnPrimary, flex: 1, padding: '10px' }}>
                    <Check size={14} /> {busy === 'google-launch' ? 'MarkingΓÇª' : 'Mark as Ready'}
                  </button>
                ) : (
                  <span style={{ flex: 1, fontSize: '12px', color: '#00e676', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', background: 'rgba(0,230,118,0.08)', border: '1px solid rgba(0,230,118,0.3)', borderRadius: '9px', padding: '10px' }}>
                    <CheckCircle2 size={14} /> Ready to publish
          ...
          {/* OPTIMIZATION RULES ΓÇö full width, below the platform reviews */}
          <div style={card}>
            ...
            {smart.killAds && (<>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', padding: '9px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <span style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>Kill if CPA over (Γé╣)</span>
      ...
      {/* ΓöÇΓöÇ 6. human review & publish ΓöÇΓöÇ */}
      <Gated locked={!platformsReady} why={!approved ? 'Approve the strategy first' : 'Mark your selected platforms as Ready first'}>
        <div style={card}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
            <button onClick={() => { setConfirmed(false); flash('Unlocked ΓÇö edit anything above, then confirm again.'); }} style={btnGhost}>Back to edit</button>
            <div style={{ flex: 1 }} />
            {platforms.meta && (
            ...
            <button onClick={() => attemptPublish(chosen)} disabled={!canPublish || busy?.startsWith('publish')}
              ...
              <Rocket size={15} /> {published ? 'Published (demo)' : busy?.startsWith('publish') ? 'PublishingΓÇª' : `Publish to ${chosen.length === 2 ? 'all selected' : chosen.length === 1 ? (chosen[0] === 'meta' ? 'Meta' : 'Google') : 'selected'}`}
            </button>
          </div>
          ...
          <p style={{ fontSize: '11.5px', color: 'var(--text-secondary)', textAlign: 'center', marginTop: '14px', background: 'rgba(255,174,0,0.05)', border: '1px solid rgba(255,174,0,0.2)', borderRadius: '9px', padding: '10px' }}>
            {published
              ? `Published to ${publishedList.map(p => (p === 'meta' ? 'Meta' : 'Google')).join(' & ')} in demo mode ΓÇö nothing was sent to a real ad account.`
              : 'Demo mode. Real publishing turns on once your Meta and Google Ads accounts are connected.'}
          </p>
        </div>
      </Gated>

      {/* ΓöÇΓöÇ mock notice ΓöÇΓöÇ */}
      <div style={{ ...card, padding: '13px 17px', display: 'flex', alignItems: 'center', gap: '11px', background: 'rgba(255,174,0,0.05)', border: '1px solid rgba(255,174,0,0.22)' }}>
        ...
        <span style={{ fontSize: '12.5px', color: 'var(--text-secondary)', lineHeight: 1.55 }}>
          <b style={{ color: '#fff' }}>Meta &amp; Google are in mock mode.</b> Their API keys aren't set up yet, so connect, launch and publish are simulated end-to-end so you can test the whole flow ΓÇö no ad account is touched and no money is spent.
        </span>
      </div>
      ...
      <div style={card}>
        ...
        {activity.length === 0 ? (
          <p style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>Nothing yet ΓÇö generate a strategy to get started.</p>
        ) : (
      ...
      {/* ΓöÇΓöÇ recently published (persists across the session; from real campaign data) ΓöÇΓöÇ */}
              ...
              return (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border, var(--border-color))', borderRadius: '10px', padding: '11px 14px' }}>
                  <span style={{ minWidth: 0 }}>
                    ...
                    <span style={{ display: 'block', fontSize: '11.5px', color: 'var(--text-secondary)' }}>{c.objective || 'ΓÇö'} ┬╖ {money(c.budget || 0)}{plats ? ` ┬╖ ${plats}` : ''}</span>
                  </span>
      ...
      {/* ΓöÇΓöÇ publish popup: connect Meta account or publish in demo mode ΓöÇΓöÇ */}
      {publishPopup && (
      ...
      {/* ΓöÇΓöÇ full review modal: the approved strategy, all in one place ΓöÇΓöÇ */}
      {whyOpen && (
      ...
      {reviewOpen && (
        <div onClick={() => setReviewOpen(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <p style={{ ...sectionHint, marginBottom: '18px' }}>Everything you approved, in one place ΓÇö read it through before publishing.</p>
            ...
            <div style={{ marginBottom: '18px' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', marginBottom: '10px' }}>
                ...
                <div style={{ flex: 1, minWidth: '230px' }}>
                  <Row k="Campaign" v={campaign?.name || 'ΓÇö'} />
                  <Row k="Objective" v={campaign?.objective || 'ΓÇö'} />
                  <Row k="Runs for" v={spec.duration_label || form.schedule} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '18px' }}>
              ...
              <div>
                ...
                <Row k="Auto-kill bad ads" v={smart.killAds ? `On ┬╖ CPA > Γé╣${smart.cpaThreshold}, freq > ${smart.frequencyCap}` : 'Off'} stack />
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { setConfirmed(true); setReviewOpen(false); flash('Reviewed and confirmed ΓÇö you can publish now.'); }}
                ...
                <Check size={15} /> I've reviewed ΓÇö confirm
              </button>
            </div>
          </div>
        </div>
