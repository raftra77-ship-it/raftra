...
export const Navbar: React.FC<{onOpenCreatorPortal?: () => void}> = ({onOpenCreatorPortal}) => {
  ...
  return (
    <motion.nav
      ...
      initial={false}
      animate={{
        // The bar is narrowed from 1240 to 1024 and its centre nudged 105px left, so
        // that the bar PLUS the Creator Marketplace pill that sits immediately after it
        // (LandingPage.tsx) reads as one centred row — measured equal at 148px of margin
        // either side. The offset is half the pill's footprint (204px + 12px gap), less
        // 3px of border/rounding. Changing either value means changing the pill's `left`
        // to match, or the row stops being centred.
        width: isCollapsed ? 78 : 'calc(100% - 48px)',
        maxWidth: isCollapsed ? 78 : 1024,
        height: isCollapsed ? 46 : 52,
        paddingLeft: isCollapsed ? 12 : 24,
        paddingRight: isCollapsed ? 12 : 24,
        left: isScrolled ? 'max(24px, calc(50% - 620px))' : 'calc(50% - 105px)',
        x: isScrolled ? '0%' : '-50%'
      }}
      transition={{
        type: 'spring',
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: 0.2, ease: 'easeOut' }}
        // marginLeft: the centre-links group and this one are separate flex containers,
        // so nothing separates them by default. At the old 1240 width there was enough
        // slack to hide that; at 1024 the last centre link ("About Us") ends up flush
        // against "Creator Onboarding". 24px keeps the rhythm close to the links' own
        // 28px gap.
