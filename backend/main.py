...
# Brand <-> creator chat no longer has a socket of its own. The /ws/chat/{room} relay that
# lived here was unauthenticated and keyed on a guessable "ws{workspace}_{handle}" room, and it
# stored nothing. Messages are now rows written through /api/inbox (inbox_routes.py) and pushed
# to exactly the two parties over the authenticated /ws socket above.
